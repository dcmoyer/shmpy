...
std::unique_ptr<std::vector<double>> ProjectSparseSamplesSeq(
    int order, const std::vector<Eigen::Vector3d>& dirs, 
    const std::vector<double>& values) {
  CHECK(order >= 0, "Order must be at least zero.");
  CHECK(values.size() % dirs.size() == 0,
      "Directions length must evenly divide values length.");
  unsigned int number_of_seqs = values.size() / dirs.size();
  unsigned int seq_length = GetCoefficientCount(order);
  ...
  // corresponding row in b is the value in @values.
  std::unique_ptr<std::vector<double>> coeffs(new std::vector<double>());
  coeffs->assign(seq_length * number_of_seqs, 0.0);

  Eigen::MatrixXd basis_values(dirs.size(), coeffs->size());
  Eigen::MatrixXd func_values(dirs.size(), number_of_seqs);

  double phi, theta;
  for (unsigned int i = 0; i < dirs.size(); i++) {
    
    for( unsigned int iter = 0; iter < number_of_seqs; ++iter ){
      func_values(i,iter) = values[i + seq_length*iter];
    }
  ...
  Eigen::MatrixXd soln = basis_values.jacobiSvd(
      Eigen::ComputeThinU | Eigen::ComputeThinV).solve(func_values);
  std::cout << "End of SVD" << std::endl;
  std::cout << soln.rows() << " " << soln.cols() << std::endl;

  // Copy everything over to our coeffs array
  for( unsigned int iter = 0; iter < number_of_seqs; ++iter ){
    for (unsigned int i = 0; i < seq_length; i++) {
      (*coeffs)[i + seq_length*iter] = soln(i,iter);
    }
  }
  //}
  return coeffs;
